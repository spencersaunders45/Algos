...
function santasNaughtyList(persons, badHabit) {
  ...
  let habitIdx = 0;
  let personIdx = 0;
  while(personIdx < persons.length){
    if(persons[personIdx].habits[habitIdx] == badHabit){
      naughtyList.push(persons[personIdx].firstName + " " + persons[personIdx].lastName);
    }

    habitIdx++;

    if(habitIdx >= persons[personIdx].habits.length){
      personIdx++;
      habitIdx = 0;
    }
  }
